function HealSound( keys )
	...
	local fullheal = ( (caster:GetBaseDamageMax() - caster:GetBaseDamageMin()) * caster:GetTeamNumber() ) * heal_percent
	...
	if caster:IsIllusion() == false then
		if ability:GetCooldownTimeRemaining() == 0 then
			...
			local emp_explosion_effect = ParticleManager:CreateParticle("particles/generic_gameplay/generic_lifesteal.vpcf",  PATTACH_ABSORIGIN, caster)
