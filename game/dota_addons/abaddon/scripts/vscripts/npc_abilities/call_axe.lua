function ApplyBuffs(event)
	...
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_beserkers_call_buff", {duration = duration})
	...
	if abiShadow and abiShadow.shadow and IsValidEntity(abiShadow.shadow) and abiShadow.shadow:IsAlive() then
		ability:ApplyDataDrivenModifier(caster, abiShadow.shadow, "modifier_beserkers_call_buff", {duration = duration})
	end
	...
	if abiArt and abiArt.shadows then 
		for _,unit in pairs(abiArt.shadows) do
			if IsValidEntity(unit) and unit:IsAlive() then
				ability:ApplyDataDrivenModifier(caster, unit, "modifier_beserkers_call_buff", {duration = duration})
			end
		end
	end
end
