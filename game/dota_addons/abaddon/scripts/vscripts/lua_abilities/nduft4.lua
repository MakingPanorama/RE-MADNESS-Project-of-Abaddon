...
function nduft4_lua:OnSpellStart()

	self.meleecreeps = self.meleecreeps + 1

	local unit = CreateUnitByName("Team_Healing_Ward", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	unit:SetDeathXP( 0 )
...
function nduft4_lua:OnUpgrade()
	...
	if not self.Init then
		...
		self.meleecreeps = 0;
		ListenToGameEvent("entity_killed", nduft4_lua.WatchmeleecreepDeaths, self)
...
function nduft4_lua:OnOwnerSpawned()

	self:StartSpawner()

end

function nduft4_lua:StartSpawner()

	self:SetThink("Makemeleecreeps", self, self:GetCooldownTimeRemaining())

end

function nduft4_lua:ShouldSpawn()
...
function nduft4_lua:Makemeleecreeps()
...
function nduft4_lua:WatchmeleecreepDeaths( event )
