...
function nduft_lua:OnSpellStart()

	self.meleecreeps = self.meleecreeps + 1

	local unit = CreateUnitByName("npc_dota_unit_melee_footman", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	unit:SetDeathXP( 0 )
...
function nduft_lua:OnUpgrade()
	...
	if not self.Init then
		...
		self.meleecreeps = 0;
		ListenToGameEvent("entity_killed", nduft_lua.WatchmeleecreepDeaths, self)
...
function nduft_lua:OnOwnerSpawned()

	self:StartSpawner()

end

function nduft_lua:StartSpawner()

	self:SetThink("Makemeleecreeps", self, self:GetCooldownTimeRemaining())

end

function nduft_lua:ShouldSpawn()
...
function nduft_lua:Makemeleecreeps()
...
function nduft_lua:WatchmeleecreepDeaths( event )
