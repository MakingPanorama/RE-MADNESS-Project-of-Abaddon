...
function modifier_brewmaster_ferocity_lua_int:GetModifierPhysicalArmorBonus()
	return self:GetStackCount()
end

function modifier_brewmaster_ferocity_lua_int:GetModifierConstantHealthRegen()
	return self:GetStackCount()
end
...
function modifier_brewmaster_ferocity_lua_int:OnIntervalThink(event)
	if IsServer() and self:GetParent():IsAlive() then
		...
		self.bonus_all_per_unit = self:GetAbility():GetSpecialValueFor("bonus_ar_he")
