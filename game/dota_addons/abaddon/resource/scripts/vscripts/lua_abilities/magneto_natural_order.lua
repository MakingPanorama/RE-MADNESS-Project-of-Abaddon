...
function modifier_magneto_natural_order_debuff:OnCreated() if IsServer() then if self:GetParent():GetPhysicalArmorValue(false) > 0 then self.armor_red = self:GetParent():GetPhysicalArmorValue( false ) * (self:GetAbility():GetSpecialValueFor("armor_reduction_pct")/100) * -1 end end end
