...
function nduft3_lua:OnSpellStart()

	self.meleecreeps = self.meleecreeps + 1

	local unit = CreateUnitByName("npc_dota_unit_melee_footman3", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	unit:SetDeathXP( 0 )
...
function nduft3_lua:OnUpgrade()
	...
	if not self.Init then
		...
		self.meleecreeps = 0;
		ListenToGameEvent("entity_killed", nduft3_lua.WatchmeleecreepDeaths, self)
...
function nduft3_lua:OnOwnerSpawned()

	self:StartSpawner()

end

function nduft3_lua:StartSpawner()

	self:SetThink("Makemeleecreeps", self, self:GetCooldownTimeRemaining())

end

function nduft3_lua:ShouldSpawn()
...
function nduft3_lua:Makemeleecreeps()
...
function nduft3_lua:WatchmeleecreepDeaths( event )
