function Spawn( entityKeyValues )
	Timers:CreateTimer(function()
        ...
        else ability_usage_1 () ability_usage_2 () return 1                --Repeat
        end
    end
    )
    Timers:CreateTimer(function()
...
function ability_usage_1 ()
    local ability = thisEntity:FindAbilityByName("bloodseeker_blood_bath")
...
function ability_usage_2 ()
    local ability = thisEntity:FindAbilityByName("bloodseeker_rupture") 
    local manacost = ability:GetManaCost(ability:GetLevel())
    local current_mana = thisEntity:GetMana()
    if ability:IsCooldownReady() and current_mana >= manacost then
        local caster = thisEntity
