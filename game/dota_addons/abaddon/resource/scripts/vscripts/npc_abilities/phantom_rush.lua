LinkLuaModifier( "modifier_movespeed_cap", "npc_abilities/modifier_movespeed_cap.lua" ,LUA_MODIFIER_MOTION_NONE )
	...
	if target ~= null and ability:IsCooldownReady() then
		...
		if caster:GetTeam() ~= target:GetTeam() then
			...
			if distance >= min_distance and distance <= max_distance then
				...
				ability:ApplyDataDrivenModifier(caster, caster, "modifier_mental_attack", {})
...
function RemoveBuff(keys)
	...
	if target == null or target ~= ability.target then
		caster:RemoveModifierByName("modifier_mental_attack")
	end
end
