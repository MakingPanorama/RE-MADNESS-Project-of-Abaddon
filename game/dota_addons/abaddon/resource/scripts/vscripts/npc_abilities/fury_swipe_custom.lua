function BearSwipes( keys )
	...
	local multiplier_stats = caster:GetPrimaryStatValue() * all_stats
	...
	if target:HasModifier( modifierName ) then
		...
		if current_stack < max_stacks then
			...
			SendOverheadEventMessage(nil, OVERHEAD_ALERT_CRITICAL, target, bonus_damage * current_stack + multiplier_stats , nil)
		else
			ability:ApplyDataDrivenModifier(caster, target, modifierName, { Duration = duration } )
		end
	else
