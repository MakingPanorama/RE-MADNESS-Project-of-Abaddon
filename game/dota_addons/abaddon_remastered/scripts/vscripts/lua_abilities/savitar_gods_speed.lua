...
LinkLuaModifier( "modifier_savitar_gods_speed",	"lua_abilities/savitar_gods_speed.lua", LUA_MODIFIER_MOTION_NONE )
...
function modifier_savitar_gods_speed:OnTakeDamage( params )
	if IsServer() then
			if params.unit == self:GetParent() then
					...
					local lifesteal_fx = ParticleManager:CreateParticle("particles/items_fx/healing_clarity.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
