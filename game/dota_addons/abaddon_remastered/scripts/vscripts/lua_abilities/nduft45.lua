...
function nduft45_lua:OnSpellStart()

	self.meleecreeps = self.meleecreeps + 1

	local unit = CreateUnitByName("npc_dragon_knight", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	unit:SetDeathXP( 0 )
...
function nduft45_lua:OnUpgrade()
	...
	if not self.Init then
		...
		self.meleecreeps = 0;
		ListenToGameEvent("entity_killed", nduft45_lua.WatchmeleecreepDeaths, self)
...
function nduft45_lua:OnOwnerSpawned()

	self:StartSpawner()

end

function nduft45_lua:StartSpawner()

	self:SetThink("Makemeleecreeps", self, self:GetCooldownTimeRemaining())

end

function nduft45_lua:ShouldSpawn()
...
function nduft45_lua:Makemeleecreeps()
...
function nduft45_lua:WatchmeleecreepDeaths( event )
