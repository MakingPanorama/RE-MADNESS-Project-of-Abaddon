...
function nduft2_lua:OnSpellStart()

	self.meleecreeps = self.meleecreeps + 1

	local unit = CreateUnitByName("npc_dota_unit_melee_footman2", self:GetCaster():GetAbsOrigin(), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	unit:SetDeathXP( 0 )
...
function nduft2_lua:OnUpgrade()
	...
	if not self.Init then
		...
		self.meleecreeps = 0;
		ListenToGameEvent("entity_killed", nduft2_lua.WatchmeleecreepDeaths, self)
...
function nduft2_lua:OnOwnerSpawned()

	self:StartSpawner()

end

function nduft2_lua:StartSpawner()

	self:SetThink("Makemeleecreeps", self, self:GetCooldownTimeRemaining())

end

function nduft2_lua:ShouldSpawn()
...
function nduft2_lua:Makemeleecreeps()
...
function nduft2_lua:WatchmeleecreepDeaths( event )
