...
function DoHealthCost( event )
    ...
    local health_cost = ability:GetLevelSpecialValueFor( "health_cost_pct" , ability:GetLevel() - 1  )
    local mult = ability:GetLevelSpecialValueFor( "mult" , ability:GetLevel() - 1  )
    local health_value = caster:GetHealth()
    local health_max = caster:GetMaxHealth()
    local mag_resist = target:GetBaseMagicalResistanceValue()
    local new_health = (health_value - ((health_max * 0.01) *(health_cost + ((mag_resist * 0.01)*mult)))
